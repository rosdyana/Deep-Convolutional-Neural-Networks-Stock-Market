...
def conv2d(input, output_dim,
    ...
    with tf.variable_scope(name):
        w = tf.get_variable('w', [conv_h, conv_w, input.get_shape().as_list()[-1], output_dim],
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
        c = tf.nn.conv2d(input, w, strides=[
                         1, conv_s, conv_s, 1], padding=padding)
...
def deconv2d(input, output_dim,
             ...
             stddev=0.02, bias=False):
    with tf.variable_scope(name):
        batch, input_h, input_w, input_dim = [
        ...
        if bias:
            b = tf.get_variable(
                'b', [input_dim], initializer=tf.constant_initializer(0.0))
            return c + b
    ...
    with tf.variable_scope(name):
        ...
        if is_2d:
            mean, variance = tf.nn.moments(
                input, axes=[0, 1, 2], keep_dims=False)
        else:
...
def fully_connected(input, output_dim, name="fc", stddev=0.02):
    with tf.variable_scope(name):
        unfolded_dim = ft.reduce(
            lambda x, y: x * y, input.get_shape().as_list()[1:])
        w = tf.get_variable('w',
