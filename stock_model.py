    ...
    def prediction(self):
        with tf.variable_scope("model") as scope:
            ...
            with tf.variable_scope("conv_1"):
                output = relu(
                    conv1d(input_image, self.filter_num, name='conv_1'))
                layers.append(output)

            # conv_2 - conv_6
            layer_specs = [
                # conv_2: [batch, 64, ngf] => [batch, 32, ngf * 2]
                (self.filter_num * 2, 0.5),
                # conv_3: [batch, 32, ngf * 2] => [batch, 16, ngf * 4]
                (self.filter_num * 4, 0.5),
                # conv_4: [batch, 16, ngf * 4] => [batch, 8, ngf * 8]
                (self.filter_num * 8, 0.5),
                # conv_5: [batch, 8, ngf * 8] => [batch, 4, ngf * 8]
                (self.filter_num * 8, 0.5),
                # conv_6: [batch, 4, ngf * 8] => [batch, 2, ngf * 8]
                (self.filter_num * 8, 0.5)
            ]
...
def main():
    ...
    # Import data
    db = load_stock_data("data/{}/".format(args.ticker))
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(3000):
            ...
            if i % 100 == 0:
                ...
                accuracy = sess.run(
                    model.accuracy, {image: images_eval, label: labels_eval, dropout: 1.0})
                print('step %d, accuracy %g' % (i, accuracy))
            sess.run(model.optimize, {image: images,
                                      label: labels, dropout: 0.5})
